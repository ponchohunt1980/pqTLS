...
int crypto_sign_keypair(unsigned char *pk, unsigned char *sk) {
  ...
  crh(tr, pk, CRYPTO_PUBLICKEYBYTES_DILI);
                ...
                const unsigned char *sk)
{
  ...
  for(i = 1; i <= mlen; ++i)
    sm[CRYPTO_BYTES_DILI + mlen - i] = m[mlen - i];
  for(i = 0; i < CRHBYTES; ++i)
    sm[CRYPTO_BYTES_DILI - CRHBYTES + i] = tr[i];

  /* Compute CRH(tr, msg) */
  crh(mu, sm + CRYPTO_BYTES_DILI - CRHBYTES, CRHBYTES + mlen);
                     ...
                     const unsigned char *pk)
{
  ...
  if(smlen < CRYPTO_BYTES_DILI)
    goto badsig;

  *mlen = smlen - CRYPTO_BYTES_DILI;
  ...
  if(sm != m)
    for(i = 0; i < *mlen; ++i)
      m[CRYPTO_BYTES_DILI + i] = sm[CRYPTO_BYTES_DILI + i];

  crh(m + CRYPTO_BYTES_DILI - CRHBYTES, pk, CRYPTO_PUBLICKEYBYTES_DILI);
  crh(mu, m + CRYPTO_BYTES_DILI - CRHBYTES, CRHBYTES + *mlen);
